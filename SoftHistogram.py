...
class SoftHistogram(nn.Cell):
    ...
    def __init__(self,n_features,n_examples,num_bins,quantiles=False):
        ...
        self.bin_centers_conv = nn.Conv1d(self.in_channels,self.num_bins*self.in_channels,kernel_size=1,group=self.in_channels,has_bias=True,weight_init=Constant(1))
        ...
        self.bin_widths_conv = nn.Conv1d(self.num_bins*self.in_channels,self.num_bins*self.in_channels,kernel_size=1,group=self.num_bins*self.in_channels,has_bias=True,bias_init=Constant(1))
        ...
        self.hist_pool = nn.AvgPool1d(n_examples,stride=1,pad_mode='pad',padding=1) #average by row
        ...
        bin_centers = -1/self.num_bins * (mindspore.numpy.arange(self.num_bins).float() + 0.5)
        self.cat = ops.Concat()
        self.bin_centers_conv.bias= mindspore.Parameter(self.cat(self.in_channels*[bin_centers]), requires_grad=True)
        bin_width = -self.num_bins*2 
        self.bin_widths_conv = nn.Conv1d(self.num_bins*self.in_channels,self.num_bins*self.in_channels,kernel_size=1,group=self.num_bins*self.in_channels,has_bias=True,bias_init=Constant(bin_width))
    ...
    def constrain_bins(self,xx):
        ...
        n,c,l = xx.size()
        xx_sum = xx.reshape(n, c//self.numBins, self.numBins, l).sum(2) + mindspore.Tensor(10e-6)
        xx_sum = 	mindspore.ops.repeat_elements(xx_sum,self.numBins,dim=1)
        xx = xx/xx_sum  
        return xx
        
        
class SoftHistogramRBF(nn.Cell):
    """
    This class computes a histogram based in convolutional layers. I have followed the paper:
    ...
    def __init__(self,n_features,n_examples,num_bins,quantiles=False):
        ...
        self.bin_widths_conv = nn.Conv1d(self.num_bins*self.in_channels,self.num_bins*self.in_channels,kernel_size=1,group=self.num_bins*self.in_channels,has_bias=False)
        ...
        #Compute initial bin centers and bin sizes, this can change during backpropagation
        bin_centers = -1/self.num_bins * (mindspore.numpy.arange(self.num_bins).float() + 0.5)
        self.bin_centers_conv.bias= mindspore.Parameter(mindspore.ops.Concat(self.in_channels*[bin_centers]), requires_grad=True)
        bin_width = self.num_bins*2
        self.bin_widths_conv.weight.data.fill_(bin_width)
        self.abs=mindspore.ops.Abs()
    ...
    def construct(self,input):
        ...
        input = mindspore.ops.Exp(-(input**2))
        # print(input.size())
        #Enforce sum to one constraint
        # Add small positive constant in case sum is zero
        #if(self.normalize_bins):
